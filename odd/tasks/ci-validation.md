...
- 2026-09-18 Pull requests #3 and #4 merged by the user (`main` at `7bda785`). Pull request #4 passed `test`, `lint`, `build` and `typecheck` on GitHub while targeting a feature branch, confirming the trigger without a base-branch filter.
- 2026-09-18 Post-merge review follow-ups on `fix/ci-review-followups`. Three Codex P2 comments had arrived after the pull requests were reported ready; each was verified against the code before accepting. (1) `pnpm check` omitted `pnpm --filter studio format:check` although the CI `lint` job runs it; added, README aligned, and every CI `run:` step cross-checked against the chain. The remaining difference is intentional and now documented: CI builds with `CONTENT_FALLBACKS=true`, the local check builds strictly against real content. (2) Flat-config globals merge, so the Node globals block (`**/*.ts`) also matched the virtual inline-script files (`**/*.astro/*.ts`) and browser scripts resolved `process`, `Buffer` and `__dirname`; `no-undef` was also off for those files and TypeScript cannot guard them because `@types/node` is ambient. The Node block now ignores the virtual files and the browser block re-enables `no-undef`. Evidence: `--print-config` for `src/components/Header.astro/1_1.ts` no longer lists the Node globals, keeps `document` and `window`, and shows `no-undef` as error; a temporary `process.env` read in the header script fails with `'process' is not defined  no-undef` and was reverted. (3) The `CONTENT_FALLBACKS` integration test was a tautology because Vitest runs with `DEV` true and the rule is `dev || flag === 'true'`. The decision is now the exported pure function `resolveFallbackPolicy({ option, dev, flag })` with a nine-case truth table (exact `'true'` match, option overrides, dev default), and the environment reader is a thin shell. Experiment recorded in the test file: `vi.stubEnv('DEV', false)` changes `import.meta.env.DEV` inside the test module but not inside `home.ts`, so the integration-level variant cannot be isolated and the tautological test was deleted. Mutation: removing `|| flag === 'true'` fails one test. Evidence: `pnpm test` 4 files and 55 tests; `pnpm lint`, `pnpm format:check`, `pnpm typecheck`, `pnpm --filter studio format:check` and `pnpm check` exit 0. Native risk assessment `medium` (executable change in `eslint.config.mjs`).
- Process correction: review comments are checked before a pull request is reported as ready, not only the CI status; the review bot posts a few minutes after each push.

## Next step

Open the follow-up pull request from `fix/ci-review-followups` into `main`. Then `media-interactions` and `contact-conversion`.
