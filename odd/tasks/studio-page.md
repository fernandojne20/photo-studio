...
- 2026-09-18 Review fixes (PR #2, two Codex P2 comments, both verified against the code before accepting). Hero: `SanityImage` and the mobile `<source>` were built without aspect ratios, so the CDN never applied the editor's hotspot and the browser center-cropped; both now pass the mockup frame ratios (`1366 / 850`, `393 / 560`), the desktop image also feeds a hotspot-aware portrait crop on phones when no mobile image is uploaded, and a 1920w candidate was added. Portfolio grid: `sizes` claimed a fixed 455px although the grid is always a third of the viewport; now `sizes="33vw"` with a 1280w candidate. Same ceiling fixed on the biography portrait (added 1600w). Evidence: `astro check` 0 errors; build complete; built hero URL carries `rect=…&w=1920&h=1195&fit=crop`, mobile source carries `h=570&fit=crop`, portfolio images report `sizes="33vw"` with a `1280w` candidate.
