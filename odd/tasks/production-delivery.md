...
- `dist/client/_headers` is GENERATED by an `astro:build:done` hook from the pure, tested `src/lib/security-headers.mjs`, never committed; a manual `public/_headers` fails the build, because the hook would silently discard it. The module is plain `.mjs` because `astro.config.mjs` must not import TypeScript. The adapter's own hook runs first (Astro unshifts the adapter to the front of the integration list) and only writes the `/_astro/*` cache rule, which the builder reproduces. `_headers` never applies to Worker responses: `/api/contact` sets its own.
- Content Security Policy: Astro's `security.csp` computes the per-build hashes and can only emit a `<meta>` element here (`@astrojs/cloudflare` does not declare `adapterFeatures.staticHeaders`). That element sits after the JSON-LD block and the font style blocks, and a meta-delivered policy only governs what follows it, so the hook ALSO delivers the policy as one real header: it reads the meta of every built page, unions the script and style hashes, appends `frame-ancestors 'none'` (which a meta element cannot carry) and fails the build on a missing meta, on pages whose other directives differ, or on a line over Cloudflare's 2,000 characters (about 1,080 today, about 47 more per hash). `script-src` keeps `'self'` because Astro's module scripts are external files without `integrity`; `'strict-dynamic'` would need a nonce, impossible for static output. The one exception is `style-src-attr 'unsafe-inline'` for the blur-up placeholders (one distinct `data:` background per photo, 15 on today's content); a browser without that directive only loses the blur-up, never the photo.
- A real `404.astro` exists because the platform serves `404.html` for every unmatched path WITH the headers, while without it unmatched paths answered a bare, header-less 404. It is `noindex` with no canonical in both modes, and the build fails if it ever becomes indexable.
...
- [x] PD-04 Security and caching headers for static assets with a Content Security Policy verified in a browser (Turnstile, PhotoSwipe, Sanity images, fonts), plus `X-Robots-Tag: noindex` for non-indexable builds and a Spanish 404 page.
...
- [ ] PD-06 Performance budgets and automated verification in CI: size budgets on `dist/client`, documented thresholds, and assertions on the built output in BOTH indexing modes (acceptance test: rewiring `sitemap.xml.ts` to ignore the URL must fail CI; a set but invalid `PUBLIC_SITE_URL` must fail the build). For PD-04 it must also assert, on the built output, that the policy in the page and in `_headers` has no `'unsafe-inline'` or `'unsafe-eval'` in `script-src` and no origin outside the allowlist, that there is exactly one policy header, and that the 404 page is `noindex` in the indexable mode.
...
- 2026-09-19 PD-04 by a delegated writer. Headers on every static response: `X-Content-Type-Options`, `Referrer-Policy: strict-origin-when-cross-origin`, a `Permissions-Policy` that turns off eighteen features the site never uses (`ambient-light-sensor` is left out because Chromium rejects the name), `X-Frame-Options: DENY`, `Cross-Origin-Opener-Policy: same-origin`, `Strict-Transport-Security: max-age=15552000` without `includeSubDomains` or `preload` (the domain is not bought yet), the policy header, and `X-Robots-Tag: noindex, nofollow` on non-indexable builds, decided from the built output (`sitemap.xml` exists only when indexable, cross-checked against the homepage's robots meta).
- Verified against the BUILT site served by `wrangler dev --local` (the dev server applies neither `_headers` nor the production policy), by the writer and again by an independent reviewer: zero `securitypolicyviolation` events for load at 1366 and 393, a full scroll, the lightbox, the mobile menu, the form, Turnstile with the always-pass and the forced-interactive keys (the challenge completes under `fullscreen=()`, `autoplay=()` and COOP) and the 404 page. Blocked, each with a violation: an inline script, a script injected as the first child of `<head>`, a `<style>` block, an inline event handler, a foreign script, image, `fetch` and form action, and framing from another origin. Parent spot check with `curl` on the served build: `/`, `/no-existe`, `/foo/bar.png`, an `/_astro` file and `/robots.txt` carry the full set (the `/_astro` file keeps its immutable cache rule), unmatched paths answer 404 with the Spanish page, there is exactly one policy header of 1,083 characters, and `/api/contact` keeps only its own headers.
- Independent review before the pull request, no blockers. Fixed: the late meta element (the policy is now also a real header), header-less unmatched paths (the 404 page), a policy with no test at all (adding `'unsafe-inline'` and a foreign origin passed everything; the constants now live in the tested module with explicit rules), a self-referential permissions test, the silent overwrite of a manual `_headers`, and oversized comments. The writer disclosed that nothing caught the 404 page becoming indexable; the parent added the build guard and proved it fails an indexable build. Declined: `Cross-Origin-Resource-Policy` (link-preview crawlers fetch the sharing image cross-origin), `X-Permitted-Cross-Domain-Policies`, `Origin-Agent-Cluster`, `'strict-dynamic'`, replacing the style-attribute exception. Mutations caught: fourteen across the writer, the reviewer and the parent. `pnpm check` exit 0, 18 files and 406 tests. Native risk assessment `medium`.
- 2026-09-19 Review bot on the pull request, two valid findings. (1) On the white 404 page the header was invisible: it is a white overlay made for the dark hero, and neither the writer nor the reviewer had looked at it. `Header.astro` gains a `solid` variant for light pages without a hero (in the flow, text color, no current item, in-page anchors pointing back to the homepage); verified in the browser at 1366 and 393, the mobile menu included, and the homepage header is unchanged. (2) The repository fallback content uses remote placeholder photos from `picsum.photos`, which `img-src` blocks. The production policy is NOT widened for a development convenience: the deploy build is strict and never emits them. Instead the build hook lists image origins outside `img-src` (`findBlockedImageOrigins`, pure and tested) and warns that such a build is not deployable; proven on a fallback build (warning) and on real content (silent).
- Worth knowing: a local `.dev.vars` overrides even shell variables at BUILD time for `PUBLIC_TURNSTILE_SITE_KEY` (the adapter assigns it onto `process.env`); HSTS is a no-op on `workers.dev` because the whole `.dev` zone is preloaded.

## Next step

PD-05 on `feat/analytics-hooks`.
